...
(function() {
	...
	Sprite_Base.prototype.updateSpriteLoopAnimations = function() {
		...
		if (object) {
			if(object._addLoopAnimation) object._addLoopAnimation.forEach(function(data)    { this.addLoopAnimation(data)    }, this);
			if (object._removeLoopAnimation) object._removeLoopAnimation.forEach(function(data) { this.removeLoopAnimation(data) }, this);
	...
	Sprite_Base.prototype.addStateAnimations = function(oldStates, newStates, show) {
		newStates.forEach(function (state) {
			if (!oldStates.contains(state) || $dataStates[state].meta.stackableState) this.addStateAnimationsData($dataStates[state], show);
		}, this)
	};
	
	Sprite_Base.prototype.addStateAnimationsData = function (state, show) {
		//var stack = 0;
		//if (state.meta.stackableState && this._battler._stateStacks[state.id] > 0) {
		//	console.log(this._battler._stateStacks[state.id], state);
		//	stack = 1
		//}
		Object.keys(state.loopAnimation).forEach(function (type) {
			var data = state.loopAnimation[type];
			if (state.meta.stackableState) {
				var newId = Number(state.meta["loop animation"].trim().split(',')[0]) + this._battler._stateStacks[state.id];
				if (newId!= data.id) {
					data.id = newId;
					this.clearLoopAnimation(type);
				}
			...
			//this.refreshLoopAnimation(data, type);
		}, this);
	};
	
	Sprite_Base.prototype.removeStateAnimations = function(oldStates, newStates, show) {
		oldStates.forEach(function(state) {
			if (!newStates.contains(state) || $dataStates[state].meta.stackableState) this.removeStateAnimationsData($dataStates[state], show);
		}, this)
	};
...
//발견 : 여기서부터 상태에 따른 애니메이션 추가
	...
	Sprite_Character.prototype.updateStateLoop = function () {
		if (this._battler && this._battler._isLoopStateChanged) {
			this._battler._isLoopStateChanged = false;
			var oldStates = this._loopStatesAnim;
			var newStates = this._battler._states;
			this.removeStateAnimations(oldStates, newStates, 'map');
			this.addStateAnimations(oldStates, newStates, 'map');
			this._loopStatesAnim = this._battler._states.clone();
		}
	};
	...
	Sprite_Character.prototype.isLoopStateChanged = function () {
		return this._battler._isLoopStateChanged;
		//var oldStates = this._loopStatesAnim;
