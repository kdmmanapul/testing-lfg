...
(function() {
  ...
  QParams.stateParamsPlus = function(id) {
    ...
    QParams.stateRateParamsPlus = function (id) {
        return this._rates['states'][id] || {};
    };
  ...
  DataManager.extractQData = function(data, object) {
    ...
    if (data.qmeta['rates']) {
      ...
      var value = QParams.stringToRatesObj(data.qmeta['rates']);
        if (object === $dataStates) {
        QParams._rates.states[data.id] = value;
        } else if (object === $dataWeapons) {
        QParams._rates.weps[data.id] = value;
        } else if (object === $dataArmors) {
        QParams._rates.armor[data.id] = value;
        }
    }
  ...
  Game_BattlerBase.prototype.initMembers = function() {
    Alias_Game_BattlerBase_initMembers.call(this);
      ...
      this._rParamPlus = {};
      for (var i = 0; i < 17; i++) {
          this._oParamPlus[i] = 0;
      };
      QParams._custom.forEach(function (param, index) {
        this._cParamPlus[index] = Number(param.default) || 0;
        this._oParamPlus[index + 17] = 0;
    }.bind(this))
  };
  ...
  Game_BattlerBase.prototype.param = function(paramId) {
      ...
      value += this.getOParamPlus(paramId, currentValue);
  ...
  Game_BattlerBase.prototype.xparam = function(xparamId) {
    var value = Alias_Game_BattlerBase_xparam.call(this, xparamId);
      value += this.getRParamPlus(xparamId, 'xParam');
    return value;
  };
  ...
  //};
    ...
    Game_BattlerBase.prototype.getOParamPlus = function (paramId, currentValue) {
        var value = this._oParamPlus[paramId];
        return Number(value) || 0;
    }

    Game_BattlerBase.prototype.addStateParams = function (stateId) {
        var params = Object.keys(QParams.stateParamsPlus(stateId));
            for (var i = 0; i < params.length; i++) {
                this._oParamPlus[params[i]] += this.evalParamFormula(QParams.stateParamsPlus(stateId)[params[i]], 0);
        }
        params = Object.keys(QParams.stateRateParamsPlus(stateId))
        for (var i = 0; i < params.length; i++) {
            if (!this._rParamPlus[params[i]]) this._rParamPlus[params[i]] = [];
            if (this._rParamPlus[params[i]].indexOf(stateId) < 0) this._rParamPlus[params[i]].push(stateId);
        }
        return;
    };

    Game_BattlerBase.prototype.removeStateParams = function (stateId) {
        var params = Object.keys(QParams.stateParamsPlus(stateId));
        for (var i = 0; i < params.length; i++) {
            this._oParamPlus[params[i]] = (this._oParamPlus[params[i]] * 100 - this.evalParamFormula(QParams.stateParamsPlus(stateId)[params[i]], 0)* 100) / 100;
        }
        params = Object.keys(QParams.stateRateParamsPlus(stateId))
        for (var i = 0; i < params.length; i++) {
            if (!this._rParamPlus[params[i]]) this._rParamPlus[params[i]] = [];
            this._rParamPlus[params[i]].splice(this._rParamPlus[params[i]].indexOf(stateId), 1);
        }
        return;
    };

    Game_BattlerBase.prototype.addPassiveStateParams = function (passiveStates) {
        for (var i = 0; i < passiveStates.length; i++) {
            var params = Object.keys(QParams.stateParamsPlus(passiveStates[i]));
            for (var j = 0; j < params.length; j++) {
                this._oParamPlus[params[j]] += this.evalParamFormula(QParams.stateParamsPlus(passiveStates[i])[params[j]], 0);
            }
            params = Object.keys(QParams.stateRateParamsPlus(passiveStates[i]))
            for (var j = 0; j < params.length; j++) {
                if (!this._rParamPlus[params[j]]) this._rParamPlus[params[j]] = [];
                if (this._rParamPlus[params[j]].indexOf(passiveStates[i]) < 0) this._rParamPlus[params[j]].push(passiveStates[i]);
            }
        }
        return;
    };

    Game_BattlerBase.prototype.removePassiveStateParams = function (passiveStates) {
        for (var i = 0; i < passiveStates.length; i++) {
            var params = Object.keys(QParams.stateParamsPlus(passiveStates[i]));
            for (var j = 0; j < params.length; j++) {
                this._oParamPlus[params[j]] = (this._oParamPlus[params[j]] * 100 - this.evalParamFormula(QParams.stateParamsPlus(passiveStates[i])[params[j]], 0)* 100) / 100;
            }
            params = Object.keys(QParams.stateRateParamsPlus(passiveStates[i]))
            for (var j = 0; j < params.length; j++) {
                if (!this._rParamPlus[params[j]]) this._rParamPlus[params[j]] = [];
                this._rParamPlus[params[j]].splice(this._rParamPlus[params[j]].indexOf(passiveStates[i]), 1);
            }
        }
        return;
    };

    Game_BattlerBase.prototype.refreshStateParams = function () {
        var value = 0;
        var states = this._states;
        this._oParamPlus = {}
        for (var i = 0; i < states.length; i++) {
            var params = Object.keys(QParams.stateParamsPlus(states[i]));
            for (var j = 0; j < params.length; j++) {
                if (!this._oParamPlus[params[j]]) this._oParamPlus[params[j]] = 0;
                this._oParamPlus[params[j]] += this.evalParamFormula(QParams.stateParamsPlus(states[i])[params[j]], 0);
            }
        }
        return;
    };
  ...
  //  var states = this._states;
  //  for (var i = 0; i < states.length; i++) {
  //    var params = QParams.stateParamsPlus(states[i]);
  //    if (params[paramId]) {
  ...
  //  var value = 0;
  //  if (this.isActor()) {
  //    value += this.rateParamPlus(paramId, 405, 'states', pType);
  //      value += this.rateParamPlus(paramId, this.actorId(), 'weps', pType);
  //      value += this.rateParamPlus(paramId, this.actorId(), 'armor', pType);
  //  }
  //  //else if (this.isEnemy()) {
  ...
  //};
    ...
    Game_BattlerBase.prototype.getRParamPlus = function (paramId) {
        var value = 0;
        var params = this._rParamPlus[paramId];
        if (params) {
            for (var i = 0; i < params.length; i++) {
                value += this.rateParamPlus(paramId, params[i], 'states', 'xParam');
            }
        }
        return Number(value) || 0;
    };
  ...
  Game_BattlerBase.prototype.qParam = function(qParamId) {
    ...
    // add 8 because first 8 are mv built in params (mhp, mmp, str, ect)
      var value = this.getOParamPlus(qParamId + 8, currentValue);
      value += this.getRParamPlus(qParamId + 8);
  ...
  Game_BattlerBase.prototype.skillMpCost = function(skill) {
      var value = Alias_Game_BattlerBase_skillMpCost.call(this, skill) - this.mrc;// + this.mcc;
      if (value < 0) value = 0;
    return value;
  };

    var Alias_Game_BattlerBase_skillTpCost = Game_BattlerBase.prototype.skillTpCost;
    Game_BattlerBase.prototype.skillTpCost = function (skill) {
        var value = Alias_Game_BattlerBase_skillTpCost.call(this, skill)  * (1 - this.trc / 100);
        if (value < 0) value = 0;
        return value;
    };
    ...
    Game_Action.prototype.makeDamageValue = function(target, critical) {
      var item = this.item();
        ...
        if (this.subject() == $gamePlayer.battler()) {
			var value = baseValue * this.calcElementRate(target) * this.subject().elementRate(this.item().damage.elementId)
			if (this.item().stypeId == 1 && $gamePlayer._tripleShot.length > 0) {
				value *= 0.7;
            }
            if ($gamePlayer._onceMultipleShot) {
                value *= $gamePlayer._onceMultipleShot;
                $gamePlayer._onceMultipleShotUsed = true;
            }
        } else var value = baseValue * this.calcElementRate(target);
      if (this.isPhysical()) {
        value *= target.pdr;
        value += target.pdc;
      }
