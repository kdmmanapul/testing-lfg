...
Game_CharacterBase.prototype.setIndicators = function (fileName, animode, range, d, mode, text, id, animated, frames, frameSpeed, force, x, y) {
	if (!this._indData) this.clearIndicators();
...
Spriteset_Map.prototype.createEventIndicators = function() {
	...
	for (var i = 0; i < this._characterSprites.length; i++) {
		if (this._characterSprites[i]._character._indData) this._indicatorsField.addChild(new EventIndicators(this._characterSprites[i]));
    };
};
...
EventIndicators.prototype.loadBitmaps = function() {
	...
	if (!this.data().animated) {
		if (this.data().fileName) {
			...
			if (isGamepad) this._img[0] = ImageManager.loadPEventIndicator("pad_"+this.data().fileName);
		    else this._img[0] = ImageManager.loadPEventIndicator(this.data().fileName);
		} else {
...
EventIndicators.prototype.refreshText = function() {
	...
	if (text == 'price') {
		var price = this._spriteChar._character._eventData.item.price;
		if (!price) {
			var rarity = this._spriteChar._character._eventData.item.rarity;
			if (rarity == 2) price = 120;
			else if (rarity == 3) price = 200;
			else if (rarity == 4) price = 300;
			else price = 70;
		}
		if (price >= 300) this._text.bitmap.textColor = "#ff2222";
		else if (price >= 200) this._text.bitmap.textColor = "#ff5555";
		else if (price >= 100) this._text.bitmap.textColor = "#ff9999";
		else this._text.bitmap.textColor = "#ffffff";
		text = String(price)
	}
...
EventIndicators.prototype.updateFade = function() {
	if (this.data().visible) {
		...
		if (this.data().termin) this.opacity = 0;
		else this.opacity -= 7;
	};
};
