...
(function() {
    ...
    _defaultParam["shockwave"] = [0, 0, -1, 90, 200, 3];
    _defaultParam["glitch"] = [5, 100, 0, 0, 0, false, 8, 512, [0, 0], [0, 0], [0, 0]]
    ...
    var glitch = function (filter, cp) {
        var loc = this.getCharLoc();
        filter.slices = cp[0];
        filter.offest = cp[1];
        filter.direction = cp[2];
        filter.fillMode = cp[3];
        filter.seed = cp[4];
        filter.average = cp[5];
        filter.minSize = cp[6];
        filter.sampleSize = cp[7];
        filter.time = this._time;
        if (this._time > 10) this.erase(); // 必要ある？
    };
    _updateFilterHandler["glitch"] = glitch;
