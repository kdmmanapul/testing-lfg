...
(function() {
  ...
  Scene_Boot.prototype.start = function() {
    if (DataManager.isBattleTest() || DataManager.isEventTest()) {
      ...
      SoundManager.preloadImportantSounds();
        ...
        if (DataManager.isAnySavefileExists() && Input.isPressed('#ctlr')) {
  ...
  Game_Map.prototype.globalLock = function(charas, mode, level) {
    ...
    if (mode === 0) {
	  ...
	  if (level == 0) this._globalLocked = false;
	  else this._globalLocked = true;
  };
  ...
  Game_CharacterBase.prototype.directionToRadian = function(dir) {
    ...
    if (dir === 1) return Math.PI * 5 / 6; // Lower Left
    // if (dir === 3) return Math.PI / 4;
    if (dir === 3) return Math.PI / 6; // Lower Right
    // if (dir === 7) return Math.PI * 5 / 4;
    if (dir === 7) return Math.PI * 7 / 6; // Top Left
    // if (dir === 9) return Math.PI * 7 / 4;
    if (dir === 9) return Math.PI * 11 / 6; // Top Right
    return 0;
  };
