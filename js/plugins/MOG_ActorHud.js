     ...
     if (this.needHide(false)) {
		...
		if (this.needFade()) {
			if (this.opacity > Moghunter.ahud_fade_limit) {
                ...
                $gameSystem._fadeSkillbar = true;
			} else if (this.opacity < Moghunter.ahud_fade_limit) { this.opacity = Moghunter.ahud_fade_limit };
        } else {
            $gameSystem._fadeSkillbar = false;
            this.opacity += 10;
...
Actor_Hud.prototype.needFade = function() {
	...
	if ($gamePlayer.screen_realY() < this._hud_size[1] - 60) {return false};
...
Actor_Hud.prototype.refreshIconHalfMode = function(sprites,image,par,par_max,mode) {	
	...
	for (var i = 0; i < sprites.length; i++) {
        ...
        icon.setFrame(hp, 0, cw, ch);
        var offsetX = -sX * (par_max - 3);
		icon.x = icon.org[0] + (sX * i) - (lX * lines) + offsetX;
        icon.y = icon.org[1] + (sY * lines);
	};
};
...
Actor_Hud.prototype.create_states = function () {
    this._states = new Sprite();
    this._stateBuffs = new Sprite();
	this._stateDebuffs = new Sprite();
	this.addChild(this._stateDebuffs)
    this.addChild(this._stateBuffs)
    ...
    var overIcons = this._battler._abilities.length - 7;
    if (overIcons > 0) {
        this._states.x = -overIcons * 16;
    }
    this.refresh_states();
};
...
// * Refresh States
//==============================
Actor_Hud.prototype.refresh_states = function () { //다시켰을때 초기값 세팅
    //this._state_icon.visible = false;
	if (this._battler._abilities.length > 0) {
		for (var i = 0; i < this._battler._abilities.length; i++) {
			var currentState = $dataStates[this._battler._abilities[i]];
			this._state_icons[i] = new Sprite(this._state_img);
			this._state_icons[i]._stateInfo = new Sprite_StateInfo(currentState);
			...
			this._state_icons[i].x = this._pos_x + Moghunter.ahud_states_pos_x + i * 32 - 10;
			...
			this._state_icons[this._iconIndex].setFrame(sx, sy, 32, 32);
			this._battler.need_refresh_bhud_states = false;
			this._states.addChild(this._state_icons[i]);
			this._iconIndex++;
		}
	}
	if (this._battler._stateBuffs.length > 0) {
		for (var i = 0; i < this._battler._stateBuffs.length; i++) {
			var buff = new Sprite(this._state_img);
			var infoState = $dataStates[this._battler._stateBuffs[i]];
			buff._stateInfo = new Sprite_StateInfo(infoState);
			...
			buff.y = this._pos_y + Moghunter.ahud_states_pos_y - 105;
			buff.visible = true;
			buff.animTrigger = false;
			buff.id = this._battler._stateBuffs[i];
			buff._iconIndex = $dataStates[buff.id].iconIndex;
			var sx = buff._iconIndex % 16 * 32;
			var sy = Math.floor(buff._iconIndex / 16) * 32;
			buff.setFrame(sx, sy, 32, 32);
			this._stateBuffs.addChild(buff);
		}
	}
	if (this._battler._stateDebuffs.length > 0) {
        for (var i = 0; i < this._battler._stateDebuffs.length; i++) {
            var debuff = new Sprite(this._state_img);
            var infoState = $dataStates[this._battler._stateDebuffs[i]];
            debuff._stateInfo = new Sprite_StateInfo(infoState);
            debuff._stateInfo.visible = false;
            debuff._stateInfo._waitCount = 0;
            debuff.addChild(debuff._stateInfo);
            debuff.x = this._pos_x + Moghunter.ahud_states_pos_x + i * 32;
            debuff.y = this._pos_y + Moghunter.ahud_states_pos_y - 140;
            debuff.visible = true;
            debuff.animTrigger = false;
            debuff.id = this._battler._stateDebuffs[i];
            debuff._iconIndex = $dataStates[debuff.id].iconIndex;
            var sx = debuff._iconIndex % 16 * 32;
            var sy = Math.floor(debuff._iconIndex / 16) * 32;
            debuff.setFrame(sx, sy, 32, 32);
            this._stateDebuffs.addChild(debuff);
        }
	}
	$gamePlayer.battler()._isLoopStateChanged = true
};

Actor_Hud.prototype.refresh_newStates = function () {
    //this._state_icon.visible = false;
    this._newState = $gameSystem._ahud_newState.shift();
    if (this._newState.category == 'buff') {
        var buff = new Sprite(this._state_img);
        buff._stateInfo = new Sprite_StateInfo(this._newState.state);
        buff._stateInfo.visible = false;
        buff._stateInfo._waitCount = 0;
        buff.addChild(buff._stateInfo);
        ...
        buff._iconIndex = this._newState.state.iconIndex;
        var sx = buff._iconIndex % 16 * 32;
        var sy = Math.floor(buff._iconIndex / 16) * 32;
        buff.setFrame(sx, sy, 32, 32);
        this._stateBuffs.addChild(buff);
        this._battler._stateBuffs.push(buff.id);
        this._battler.need_refresh_bhud_states = false;
        return;
    } else if (this._newState.category == 'debuff') {
        var debuff = new Sprite(this._state_img);
        debuff._stateInfo = new Sprite_StateInfo(this._newState.state);
        debuff._stateInfo.visible = false;
        debuff._stateInfo._waitCount = 0;
        debuff.addChild(debuff._stateInfo);
        debuff.x = this._pos_x + Moghunter.ahud_states_pos_x + this._stateDebuffs.children.length * 32;
        debuff.y = this._pos_y + Moghunter.ahud_states_pos_y - 140;
        debuff.visible = true;
        debuff.animTrigger = false;
        debuff.id = this._newState.state.id;
        debuff._iconIndex = this._newState.state.iconIndex;
        var sx = debuff._iconIndex % 16 * 32;
        var sy = Math.floor(debuff._iconIndex / 16) * 32;
        debuff.setFrame(sx, sy, 32, 32);
        this._stateDebuffs.addChild(debuff);
        this._battler._stateDebuffs.push(debuff.id);
        this._battler.need_refresh_bhud_states = false;
        return;
    }
    var i = this._battler._abilities.length;
    this._state_icons[i] = new Sprite(this._state_img);
        this._state_icons[i]._stateInfo = new Sprite_StateInfo(this._newState.state);
        this._state_icons[i]._stateInfo.visible = false;
        this._state_icons[i]._stateInfo._waitCount = 0;
        this._state_icons[i].addChild(this._state_icons[i]._stateInfo);
    ...
    var sy = Math.floor(this._state_icons[i]._iconIndex / 16) * 32;
    this._state_icons[i].setFrame(sx, sy, 32, 32);
    //this.addChild(this._state_icons[i])
    this._states.addChild(this._state_icons[i]);
    this._battler._abilities.push(this._newState.state.id)
    this._iconIndex++;
    this._battler.need_refresh_bhud_states = false;
    var overIcons = this._battler._abilities.length - 7;
    if (overIcons > 0) {
...
Actor_Hud.prototype.update_buffs = function (buff) {
    var duration = this._battler._stateSteps[buff.id]
	...
	} else if (buff.opacity < 255) buff.opacity = 255;
}

Actor_Hud.prototype.update_debuffs = function (debuff) {
    var duration = this._battler._stateSteps[debuff.id]
	if (duration <= 0 || !duration) {
		this._stateDebuffs.removeChild(debuff);
		for (var i = 0; i < this._stateDebuffs.children.length; i++) {
			this._stateDebuffs.children[i].x = this._pos_x + Moghunter.ahud_states_pos_x - i * 32;
		}
	} else if (duration <= 180) {
		if (!debuff.animTrigger) {
			debuff.opacity -= 12;
			if (debuff.opacity <= 100) {
				debuff.opacity = 100;
				debuff.animTrigger = true;
			}
		} else {
			debuff.opacity += 12;
			if (debuff.opacity > 220) {
				debuff.opacity = 220;
				debuff.animTrigger = false;
			}
		}
		...
		if (!debuff.animTrigger) {
			debuff.opacity -= 5;
			if (debuff.opacity < 150) {
				debuff.opacity = 150;
				debuff.animTrigger = true;
			}
		} else {
			debuff.opacity += 5;
			if (debuff.opacity > 240) {
				debuff.opacity = 240;
				debuff.animTrigger = false;
			}
		}
	} else if (debuff.opacity < 255) debuff.opacity = 255;
}


Actor_Hud.prototype.mouseInState = function (state) {
    rx = TouchInput.x - (state.x + this._states.x);
    ry = TouchInput.y - state.y;
    inRect = rx >= 0 && ry >= 0 && rx < state.width && ry < state.height;
    if (inRect) {
        //스텟 띄우는 함수.
        state._stateInfo._waitCount++;
        if (state._stateInfo._waitCount > 30) {
            state._stateInfo.visible = true;
            if (state._stateInfo.opacity < 255) state._stateInfo.opacity += 20;
        }
    } else if (state._stateInfo._waitCount > 0) {
        state._stateInfo.visible = false;
        state._stateInfo._waitCount = 0;
        state._stateInfo.opacity = 0;
    }
}

Actor_Hud.prototype.mouseInBuff = function (state) {
    rx = TouchInput.x - state.x;
    ry = TouchInput.y - state.y;
    inRect = rx >= 0 && ry >= 0 && rx < state.width && ry < state.height;
    if (inRect) {
        //스텟 띄우는 함수.
...
Actor_Hud.prototype.mouseInDebuff = function (state) {
    rx = TouchInput.x - state.x;
    ry = TouchInput.y - state.y;
    inRect = rx >= 0 && ry >= 0 && rx < state.width && ry < state.height;
    if (inRect) {
...
Actor_Hud.prototype.update_states = function () {
    if (!this._state_icons) { return };
	if (this.need_refresh_states()) { this.refresh_newStates(); };
	if (!$gameSwitches._data[50]) {
		if (TouchInput._y > 420) {
			var x = TouchInput.x;
			var y = TouchInput.y;
			for (var i = 0; i < this._states.children.length; i++) {
				this.mouseInState(this._states.children[i])
			}
			for (var i = 0; i < this._stateBuffs.children.length; i++) {
				this.mouseInBuff(this._stateBuffs.children[i]);
				this.update_buffs(this._stateBuffs.children[i]);
			}
			for (var i = 0; i < this._stateDebuffs.children.length; i++) {
				this.mouseInDebuff(this._stateDebuffs.children[i]);
				this.update_debuffs(this._stateDebuffs.children[i]);
			}
		} else {
			for (var i = 0; i < this._stateBuffs.children.length; i++) {
				this.update_buffs(this._stateBuffs.children[i]);
...
Actor_Hud.prototype.need_refresh_states = function () {
    ...
    if ($gameSystem._ahud_newState.length > 0) {
        return true;
    }
    return false;
};
//-----------------------------------------------------------------------------
  ...
  Sprite_StateInfo.prototype = Object.create(Sprite_Base.prototype);
  Sprite_StateInfo.prototype.constructor = Sprite_StateInfo;

  Sprite_StateInfo.prototype.initialize = function(state) {
      Sprite_Base.prototype.initialize.call(this);
      this.y = -this.height;
      this._state = state;
      this._stateId = state ? state.stateId : null;
      this.visible = false;
      this.opacity = 0;
	  var maxString = this._state.message1 ? this._state.message1.length : 0;
      var fontSize = null;
      var iconWidth = 4;
      if (this._state.name.length * 24 + 32> maxString * 14) {
          maxString = this._state.name;
          fontSize = 24;
          iconWidth = 60;
      } else {
          maxString = this._state.message1
          fontSize = 14;
      }
      var nameLength = 0;
      for (var i = 0; i < maxString.length; i++) {
          if (escape(maxString.charAt(i)).length == 6) nameLength++;
          nameLength++;
      }
      this.width = nameLength/2 * fontSize + iconWidth + 4;//this.state.textWidth(this._state.name);
      this.height = 250;
      this.drawInfo();
};

Sprite_StateInfo.prototype.rarityTextColor = function (rarity) {
    switch (rarity) {
        case 2:
            return '#66cc40'
        case 3:
...
Sprite_StateInfo.prototype.createBackground = function () {
    this.bitmap = new Bitmap(this.width, this.height);
    this.bitmap.fillAll('rgba(0, 0, 0, 0.8)');
};

Sprite_StateInfo.prototype.drawInfo = function () {
...
Sprite_StateInfo.prototype.drawIcon = function (x, y) {
    var iconIndex = this._state.iconIndex;
...
Sprite_StateInfo.prototype.drawName = function (x, y) {
    this.bitmap.fontSize = 24;
    this.bitmap.textColor = this.rarityTextColor(this._state.rarity);
    this.bitmap.drawText(this._state.name, x, y, this.width, 36, 'center');
    this._realHeight = Math.max(y + 28, this._realHeight);
};

Sprite_StateInfo.prototype.drawAbsSettings = function (x, y) {
...
Sprite_StateInfo.prototype.drawDescription = function (x, y) {
    this.bitmap.fontSize = 14;
    this.bitmap.textColor = '#ffffa0';
    //this.bitmap.drawText('Desc:', x, y, this.width, 18, 'left');


    var desc = this._state.message1 ? this._state.message1 : '';
    if (this._state.message2 && this._state.message2.length > 0) desc += '\n' + this._state.message2;
	if (this._state.message3 && this._state.message3.length > 0) desc += '\n' + this._state.message3;
    var settings = {
        fontName: 'GameFont',
        fontSize: 14,
        fill: '#ffffff',
    ...
    var desc2 = this._state.message4;
    if (desc2) { 
    var settings = {
        fontName: 'GameFont',
        fontSize: 12,
    ...
    this._desc2 = new PIXI.Text(desc2, settings);
    this._desc2.x = x;
    this._desc2.y = y + this._desc.height - 1;
    this.addChild(this._desc2);
    this._realHeight = Math.max(y + this._desc.height + this._desc2.height, this._realHeight);
    } else this._realHeight = Math.max(y + this._desc.height, this._realHeight);
};

Sprite_StateInfo.prototype.drawLine = function (y) {
    this.bitmap.fillRect(2, y, this.width - 4, 2, 'rgba(255, 255, 255, 0.8)');
    this._realHeight = Math.max(y + 4, this._realHeight);
};

Sprite_StateInfo.prototype.drawData = function (x, y) {
