...
(function() {
  ...
  QMovement.tileBoxes = {
    ...
    2063: [48, 48], // Impassable A1
      2575: [48, 48],
      3584: [48, 48], // in roof
    3586: [54, 48, -6], // left roof
    3588: [54, 48], // right roof
    3590: [60, 48, -6], // up down in roof
    3592: [48, 48], // up roof
      3594: [54, 48, -6], // up left roof
    3596: [54, 48, -6], // upRight roof
    3598: [[48, 6], [6, 48], [6, 48, 42]],
    3599: [60, 48, -6],  // wall           Impassable A2, A3, A4
    3727: [48, 48]
  };
...
(function() {
  ...
  Polygon_Collider.prototype.moveTo = function(x, y) {
    ...
    Polygon_Collider.prototype.positionMoveTo = function (x, y) {
        if (x !== this.x || y !== this.y) {
            this.x = x;
            this.y = y;
            this.setBounds();
    ...
    Polygon_Collider.prototype.refreshVerticesPosition = function () {
            //var vertex = this._vertices[i];
            //vertex.x = this.x + this._baseVertices[i].x + this.ox;
            //vertex.y = this.y + this._baseVertices[i].y + this.oy;
            //if (i !== 0) {
...
(function() {
  ...
  Game_Map.prototype.canvasToMapPX = function(x) {
    ...
    return this.roundPX(originX + x1);
  };
  ...
  Game_Map.prototype.canvasToMapPY = function(y) {
    ...
    return this.roundPY(originY + y1);
  };
})();
...
(function() {
  ...
  Game_CharacterBase.prototype.canMove = function() {
      ...
      if (this._jumpCount) return false;
  ...
  Game_CharacterBase.prototype.canPass = function(x, y, dir) {
    ...
    var y1 = $gameMap.roundPYWithDirection(y, dir, dist);
        if (!this.collisionCheck(x1, y1, dir, dist, type)) {
            if (this.collidesWithAnyCharacter(type)) {
                //캐릭터와 충돌했을 경우 움직이는 자가 밀려나게)
                    ...
                    ColliderManager.getCharactersNear(collider, function (chara) {
                        ...
                        if (collided) return targetCollider = chara.collider('collision');
                    }.bind(this));
                    var x2 = collider.center.x - targetCollider.center.x;
                    var y2 = collider.center.y - targetCollider.center.y;
                    var insectRadian = Math.atan2(y2,x2)
                    var targetRatio = Math.atan2(targetCollider.height ,targetCollider.width);
                    var skillLocked = this == $gamePlayer ? $gamePlayer._skillLocked.length > 0 : false; 
                        insectRadian = QPlus.adjustRadian(insectRadian);
                        if (insectRadian >= 0 && insectRadian < targetRatio) {
                            dir = 6;
                        } else if (insectRadian >= targetRatio && insectRadian < Math.PI - targetRatio) {
                            dir = 2;
                        } else if (insectRadian >= Math.PI - targetRatio && insectRadian < Math.PI + targetRatio) {
                            dir = 4;
                        } else if (insectRadian >= Math.PI + targetRatio && insectRadian < Math.PI * 2 - targetRatio) {
                            dir = 8;
                        } else {
                            dir = 6;
                        }  
                    this.collider(type).moveTo(this._px, this._py);
                    if (!targetCollider.intersects(collider)) {
                     this.collider(type).moveTo(this._px, this._py);
                    return false;
                    }
                    x1 = $gameMap.roundPXWithDirection(x, dir, dist+3);
                    y1 = $gameMap.roundPYWithDirection(y, dir, dist+3);   
                    this.collider(type).moveTo(x1, y1);
                    if (this.collidesWithAnyTile(collider) || this.collideWithCollisionMap(type, dir) || skillLocked) {
                        this.collider(type).moveTo(this._px, this._py);
                        return false;
                    } else {
            ...
            } //else this.collider(type).moveTo(this._px, this._py);
  ...
  Game_CharacterBase.prototype.collidedWithTile = function(type, collider) {
    ...
    Game_CharacterBase.prototype.collidedWithCharacter = function (type, chara) {
        if (chara.isThrough() || chara === this) {
            return false;
        }
        if (this.team() > 0 && chara.team() == this.team()) return false;
        if ((this._transit && this._transit.length > 0) || chara._jumpCount) {
            if (chara._battler) {
                if (!chara.battler()._noTransit && !chara._noTransit) {
                    return false;
                }
            }
        }
  ...
  Game_CharacterBase.prototype.passableColors = function() {
    ...
    var colors = ['#ffffff', '#00000000', '#00ff00'];
  ...
  Game_CharacterBase.prototype.updateJump = function() {
    ...
    this._px = this._realPX = this._realX * QMovement.tileSize;
    this._py = this._realPY = this._realY * QMovement.tileSize;
  ...
  Game_CharacterBase.prototype.pixelJump = function(xPlus, yPlus) {
    ...
    Game_CharacterBase.prototype.pixelZJump = function (xPlus, yPlus, zPlus, peak) {
        ...
        this._jumpPeak = peak ? peak : zPlus;
        this._jumpCount = zPlus * 2;
        this._jumpCountHalf = zPlus;
        this.resetStopCount();
        this.straighten();
    };

    Game_CharacterBase.prototype.jumpHeight = function () {
        if (this._jumpCountHalf) {
            return (this._jumpPeak * this._jumpPeak -
                Math.pow(this._jumpPeak * Math.abs(this._jumpCount - this._jumpCountHalf) / this._jumpCountHalf, 2)) / 2;
        }
        else return (this._jumpPeak * this._jumpPeak -
  ...
  Game_CharacterBase.prototype.moveDiagonally = function(horz, vert, dist) {
    ...
    if (this.isMovementSucceeded()) {
      this._forwardRadian = this.directionToRadian(this.direction8(horz, vert));
      this._adjustFrameSpeed = false;
      this._px = $gameMap.roundPXWithDirection(this._px, horz, dist * 2);
      this._py = $gameMap.roundPYWithDirection(this._py, vert, dist);
      this._realPX = $gameMap.pxWithDirection(this._px, this.reverseDir(horz), dist);
      this._realPY = $gameMap.pyWithDirection(this._py, this.reverseDir(vert), dist);
      this.increaseSteps();
  ...
  Game_CharacterBase.prototype.fixedDiagMove = function(horz, vert, dist) {
    ...
    Game_CharacterBase.prototype.fixedRadianMove = function (radian, dist) {
        ...
        var horz = xAxis > 0.00001 ? 6 : xAxis < -0.00001 ? 4 : 0;
        var vert = yAxis > 0.00001 ? 2 : yAxis < -0.00001 ? 8 : 0;
        ...
        if (!this.isMovementSucceeded() && this.smartMove() > 1) {
            if (this == $gamePlayer) {
                if (!vert) {
                    if (this._radMoveVert == undefined) {
                        if (this.canPixelPass(this._px, this._py, 8, 8) && this.canPixelPass(this._px, this._py, 2, 8)) this._radMoveVert = 0;
                        else if (this.canPixelPass(this._px, this._py, 8, 8)) this._radMoveVert = 8;
                        else this._radMoveVert = 2;
                    }
                    vert = this._radMoveVert;
                } else if (!horz) {
                    if (this._radMoveHorz == undefined) {
                        if (this.canPixelPass(this._px, this._py, 4, 8) && this.canPixelPass(this._px, this._py, 6, 8)) this._radMoveHorz = 0;
                        else if (this.canPixelPass(this._px, this._py, 6, 8)) this._radMoveHorz = 6;
                        else this._radMoveHorz = 4;
                    }
                    horz = this._radMoveHorz;
                }
            }
            if (this.canPixelPass(this._px, this._py, horz)) {
                this.moveStraight(horz);
  ...
  Game_CharacterBase.prototype.setupColliders = function() {
    ...
    var multi = /<colliders>([\s\S]*)<\/colliders>/i.exec(notes);
      ...
      var bulletThrough = /<bulletThrough[:|=](.*?)>/i.exec(notes);
    if (multi) {
    ...
    if (single) {
      ...
      if (bulletThrough) {
          if (bulletThrough[1] == "true") this._bulletThrough = true;
          else this._bulletThrough = false;
      };
...
(function() {
  ...
  Game_Character.prototype.adjustPosition = function(xf, yf) {
    ...
    while (!this.canPixelPass(final.x, final.y, 5)) {
      final.x -= distX;
      final.y -= distY;
      dx = final.x - this._px;
      dy = final.y - this._py;
...
(function() {
  ...
  Game_Player.prototype.moveByMouse = function(x, y) {
    ...
    Game_Player.prototype.frameSpeed = function (multi) {
        var multi = multi === undefined ? 1 : Math.abs(multi);
        //if (!this._skillLocked.length > 0) {multi *= this.battler().msp;
        //}
        return this.distancePerFrame() * QMovement.tileSize * multi;
    };
  ...
  Game_Player.prototype.moveWithAnalog = function() {
	...
	Game_Player.prototype.update = function (sceneActive) {
		//if (!this._globalLocked && $gameMap._globalLocked) {
		//	this._activeSkills[this._activeSkills.length - 1].sequencer.update()
		//	return;
		//}
        if (this._globalLocked && !$gameSwitches._data[10]) {
            if (this._activeSkills.length > 0) this._activeSkills[this._activeSkills.length - 1].sequencer.update()
			return;
		}
  ...
  Game_Player.prototype.updateNonmoving = function(wasMoving) {
    ...
    } else if (this.residual().valid) this.residual().setValid(false)
  };
...
(function () {
    ...
    Game_Event.prototype.initialPosition = function () {
		...
		var z = /<z[=|:](-?[0-9]+)>/i.exec(this.comments(true)) || 0;
        if (ox) ox = Number(ox[1]) || 0;
		if (oy) oy = Number(oy[1]) || 0;
        if (z) {
            if (z[1] < 0) z = this._priorityType = -0.1;
			else z = this._priorityType = Number(z[1]);
		}
    ...
    Game_Event.prototype.moveTypeRandom = function () {
		...
		if (this._freqCount === 0 || this._typeRandomDir === null) this._typeRandomDir = Math.random() * Math.PI * 2;
		this.moveRadian(this._typeRandomDir);
    };
