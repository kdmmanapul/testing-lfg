 ...
 * MV Default: #shift, #cancel, $R1
 * @type text[]
 * @default ["#shift", "#cancel", "$R1"]
 *
 * @param Control
 * @parent Input Remap
 ...
 * @param Space
 * @parent Input Remap
 * @desc Which buttons will trigger the Space input
 * MV Default: #space, $R1
 * @type text[]
 * @default ["#space", "$R1"]
 *
 * @param 1
 * @parent Input Remap
 * @desc Which buttons will trigger the 1 input
 * MV Default: #1
 * @type text[]
 * @default ["#1", "$LEFT"]
 *
 * @param 2
 * @parent Input Remap
 * @desc Which buttons will trigger the 2 input
 * MV Default: #2
 * @type text[]
 * @default ["#2", "$UP"]
 *
 * @param 3
 * @parent Input Remap
 * @desc Which buttons will trigger the 3 input
 * MV Default: #3
 * @type text[]
 * @default ["#3", "$DOWN"]
 *
 * @param 4
 * @parent Input Remap
 * @desc Which buttons will trigger the 4 input
 * MV Default: #4
 * @type text[]
 * @default ["#4", "$RIGHT"]
 *
...
(function() {
    ...
    switch (key) {
      ...
      case 'restart':
        return Number(this.keyAt(ConfigManager.keys['restart'])) || 116;
            break;
        case 'space':
            return ConfigManager.keys['space'];
            break;
        case '1':
            return ConfigManager.keys['1'];
            break;
        case '2':
            return ConfigManager.keys['2'];
            break;
        case '3':
            return ConfigManager.keys['3'];
            break;
        case '4':
            return ConfigManager.keys['4'];
            break;
      case 'console':
  ...
  Input._setCapsLock = function(event) {
    ...
    newState[15] = false;
        ...
        if (lastState[6] == true && newState[6] == false) Input._gamepadLeftButtonReleased = true;
        else Input._gamepadLeftButtonReleased = false;
    // dpad
    //if (newState[12]) {
    //  this._dirAxesA.y = -1;
    //}
    ...
    // right stick
      ...
      this._dirAxesB.x = (axes[2] ) / max;
        this._lastUsed = 'gamepad';
    }
    if (Math.abs(axes[3]) > _THRESHOLD) {
      var sign = axes[3] > 0 ? 1 : -1;
      this._dirAxesB.y = (axes[3] ) / max;
      this._lastUsed = 'gamepad';
    }
    this._lastGamepadTriggered = null;
