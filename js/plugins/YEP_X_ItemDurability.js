  ...
  for (var n = 1; n < group.length; n++) {
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      if (line.match(/<DURABILITY:[ ](\d+)>/i)) {
          ...
          obj.durMax = Math.max(1, parseInt(RegExp.$1));
...
ItemManager.setNewIndependentItem = function(baseItem, newItem) {
  Yanfly.IDur.ItemManager_setNewIndependentItem.call(this, baseItem, newItem);
    ...
    if (baseItem.durability) baseItem.durability = baseItem.durability;
    else this.makeDurability(newItem, variance);
};

ItemManager.makeDurability = function(item, variance) {
    if (DataManager.isItem(item)) return;
    item.durability = Yanfly.Param.IDurDefaultDur;
    variance = variance || 0;
    ...
    ItemManager.durMaxChange = function (item, variance) {
        if (!item || DataManager.isItem(item)) return;
        item.durMax += variance;
        if (item.durability > item.durMax) item.durability = item.durMax;
        if (item.durMax <= 0) $gameParty.leader().durabilityBreakItem(item)
    };

    ItemManager.durabilityChange = function (item, variance) {
        if (!item || DataManager.isItem(item)) return;
        item.durability += variance;
        if (item.durability > item.durMax) item.durability = item.durMax;
        if (item.durability <= 0) $gameParty.leader().durabilityBreakItem(item)
    };
...
Game_Actor.prototype.durabilityBreakItem = function(obj) {
    if (!obj) return;
    var inventory = $gameParty._inventory;
    var bestWeapon = null;
    for (var i = 0; i < inventory.length; i++) {
        if (DataManager.isWeapon(inventory[i])) {
            if (inventory[i].durability <= 0) continue;
            if (!bestWeapon) bestWeapon = inventory[i];
            else if (obj.wtypeId == bestWeapon.wtypeId && inventory[i].rarity > bestWeapon.rarity) bestWeapon = inventory[i];
            else if (obj.wtypeId == inventory[i].wtypeId) bestWeapon = inventory[i];
        }
	}
    this.changeEquip(obj.etypeId - 1, bestWeapon);
...
Game_Actor.prototype.playDurabilityBreakSound = function(obj) {
    ...
    Game_Actor.prototype.damageAllWeaponDurability = function (value, force) {
        if (!$gameSwitches._data[50] && !force) return;
        if (!this.isStateAffected(204) && !this.isStateAffected(407)) this.damageAllDurability(value, this.weapons());
};
...
//=============================================================================
// Window_ItemActionCommand  -   ������â���� ���� Ŀ�ǵ� �߰����ִ°�
//=============================================================================
