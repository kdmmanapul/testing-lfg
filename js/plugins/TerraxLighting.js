...
(function() {
	...
	Lightmask.prototype._createBitmap = function() {
	    this._maskBitmap = new Bitmap(maxX+20,maxY+20);   // one big bitmap to fill the intire screen with black
			...
			if ($gameVariables.GetStopScript() == false) {
				...
				if ($gameVariables.GetScriptActive() == true && $gameMap.mapId() >= 0) {
					...
					if (searchdaynight.search('daynight') >= 0) {
						this._addSprite(-10, -10, this._maskBitmap); // daynight tag? yes.. then turn off the lights
						darkenscreen = true;
					} else {
						for (var i = 0; i < event_note.length; i++) {
							//if ($gameMap.events()[i]) {
								...
								if (note_command == "light" || note_command == "fire" || note_command == "daynight" || note_command == "flashlight") {
									this._addSprite(-10, -10, this._maskBitmap); // light event? yes.. then turn off the lights
									darkenscreen = true;
									break;
								}
							//}
					...
					if (darkenscreen == true) {
						...
						if (daynightset == true) {
							...
							this._maskBitmap.FillRect(0, 0, maxX + 20, maxY+20, color1);
						}
						...
						if (daynightset == false) {
							...
							this._maskBitmap.FillRect(0, 0, maxX + 20, maxY+20, tcolor);
						}
