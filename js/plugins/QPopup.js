...
(function() {
  ...
  Window_QPopup.prototype.standardPadding = function() {
    ...
    var lines = string.split(/\n|\r/);
      var largestW = 0;
      if (lines[0] == 'critical!' || lines[0] == 'weak' || lines[0] == 'strong') {
          var tempStyle = this._style;
          var indexHeight = 0;
          this.height = 0;
          for (var i = 0; i < lines.length; i++) {
              var line = this.convertEscapeCharacters(lines[i]);
              line = line.replace(/\x1b.+?\[.+?\]/, '');
              if (i != lines.length - 1) this._style = $gameSystem.qPopupPreset(line).style;
              else this._style = tempStyle;
              var w = lines[i].length * this._style.fontSize / 2;
              if (w > largestW) largestW = w + this.standardPadding() * 2;
              this.height += this._style.fontSize
          }
          this.width = largestW + this.standardPadding() * 2;
          this.height += this.standardPadding() * 2;
          this.createContents();
          for (var i = 0; i < lines.length; i++) {
              if (i != lines.length - 1) this._style = $gameSystem.qPopupPreset(lines[i]).style;
              else this._style = tempStyle;
              this.drawTextEx(lines[i], (largestW - lines[i].length * this._style.fontSize/2) / 2, indexHeight);
              indexHeight += this._style.fontSize - 4*(lines.length-1);
          }
  ...
  Window_QPopup.prototype.slidedown = function(action) {
    ...
    Window_QPopup.prototype.slidedamage = function (action) {
        var duration = Number(action[1]);
        if (this._damageTimer == 0) this._damageTimer = duration
        var distanceX = Number(action[2]);//Number(action[2]);
        var distanceY = Number(action[3]); // 디스턴스만큼 위로 점점 줄면서 올라가고, 다 올라가면 5/2만큼 내려옴. 5/3은 올라가고 5/2는 내려옴 10초면 10*5/3 올라가고 10*5/2 내려옴
        var speedX = distanceX / duration;
        var speedY = (distanceY / duration) * (this._damageTimer - duration*2/5) / duration;
        this.pivot.x += speedX;
        this.pivot.y += speedY;
        this._damageTimer--;
    };
