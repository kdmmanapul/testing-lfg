...
(function ($) {
  ...
  $.defaultMode = $._modes[$.params["Default Choice Mode"].toUpperCase()];
    $.indentX = Math.floor($.params["Choice Indent"]);
    $.indentY = 0;
  ...
  HMS.setChoiceMode = function(mode, msgId) {
    ...
    HMS.setMessageX = function (x) {
        $.indentX = x;
    };

    HMS.setMessageY = function (y) {
        $.indentY = y;
    };
  ...
  Window_ChoiceList.prototype.updatePlacement = function() {  
    TH_WindowChoiceList_updatePlacement.call(this);
      if (this._choiceMode === 1 && this._messageWindow._textState) {
      ...
      this.x = this._messageWindow.newLineX() + $.indentX;     
      var y = messageY + (textState.y) + $.indentY
