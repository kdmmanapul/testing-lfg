...
ItemManager.applyAugmentEffects = function(item, effectItem, slotId, gain) {
    ...
    if (item.passiveStates && $gameParty.leader().isEquipped(item)) $gameParty.leader().removePassiveStateParams(item.passiveStates);
    if (item.augmentSlotItems[slotId] !== 'none') {
        //if (item.passiveStates && gain) $gameParty.leader().removePassiveStateParams(item.passiveStates); //이부분 왜 지웠는지 기억안남 인챈트때문에
    ...
    if (item.passiveStates && gain && $gameParty.leader().isEquipped(item)) $gameParty.leader().addPassiveStateParams(item.passiveStates);  //이부분도 왜 지웠는지 기억안남
};
...
ItemManager.removeAugmentFromSlot = function(item, slotId) {
    ...
    $gameTemp._augmentSetting = undefined;
    //if (item.passiveStates) $gameParty.leader().removePassiveStateParams(item.passiveStates);
    return augment;
};
...
ItemManager.installAugmentToSlot = function(item, effectItem, slotId) {
    ...
    if (slotId >= 2) list = effectItem.augmentDataAttach['WGRADE1'];
    console.log("install", type, list, slotId)
    if (list && list.length > 0) {
