...
ItemManager.customizeNewIndependentItem = function(baseItem, newItem) {
	...
	//random skill �߰�
	//if (newItem.qmeta.passiveSkills) {
	//	var passiveData = newItem.qmeta.passiveSkills.split(',');
	//	for (var i = 0; i < passiveData.length; i++) passiveData[i] = Number(passiveData[i]);
	//	var randPassives = [];
	//	var randPassive;
	//	for (var i = 0; i < passiveData[2]; i++) {
	//		randPassive = passiveData[0] + Math.floor((passiveData[1] - passiveData[0] + 1) * Math.random());
	//		if (!randPassives.contains(randPassive)) randPassives.push(randPassive)
	//		else i--;
	//	}
	//	for (var i = 0; i < randPassives.length; i++) newItem.passiveStates.push(randPassives[i]);
	//}
};
...
ItemManager.setNameSuffix = function (item, value) {
	...
	if (eval(Yanfly.Param.ItemNameSpacing) && item.nameSuffix.length > 0) {
		item.nameSuffix = item.nameSuffix + ' '; // ������� ��� ���� �ݴ��. ' ' + item.nameSuffix
	}
};
...
ItemManager.updateItemName = function (item) {
	...
	item.name = fmt.format(prefix, suffix, baseName, boostText); // ������� ��� ��������. item.name = fmt.format(prefix, baseName, suffix, boostText);
};
...
Game_Party.prototype.gainItem = function (item, amount, includeEquip) {
    if (this.isInventoryPermitted(item, amount)) {
        if (DataManager.isIndependent(item)) {
            ...
            if (this.numItems(item) == amount) {
                this._inventory.push(item)
            } else if (this.numItems(item) == 0) {
                var index = this._inventory.indexOf(item);
                if (index > -1) this._inventory.splice(index, 1)
            }
        }
    }
    //if (item != null && $gameSwitches._data[11]) $gameTemp._infoPopup.push(item)
};

Game_Party.prototype.gainEquipItem = function (item, amount, includeEquip) {
    if(item) this._inventory.unshift(item)
        Yanfly.Item.Game_Party_gainItem.call(this, item, amount, includeEquip);
};
...
Game_Party.prototype.gainIndependentItem = function (item, amount, includeEquip) {
    ...
    if (amount > 0) {
        for (var i = 0; i < amount; ++i) {
            if (this.numItems(item) > 1) return arr;
                var newItem = DataManager.registerNewItem(item);
                this.registerNewItem(item, newItem);
            arr.push(newItem);
            if (item._undefinedItem) {
                if (DataManager.isWeapon(newItem)) {
                    PRG.wUndefinedEnchant(newItem);
                    //item.name = newItem.name;
                }
                else if (DataManager.isArmor(newItem)) {
                    PRG.aUndefinedEnchant(newItem);
                    //item.name = newItem.name;
                }
                item._undefinedItem = false;
                newItem._undefinedItem = false;
            }
        ...
        for (var i = 0; i < amount; ++i) {
            var index = item.id > 3000 ? this._inventory.indexOf(item) : this._inventory.indexOf(this.getInventoryMatchingBaseItem(item));
            if (index > -1) this._inventory.splice(index, 1)
            if (item.baseItemId) {
...
Game_Party.prototype.getInventoryMatchingBaseItem = function (baseItem, equipped) {
    if (!baseItem) return null;
    var group = this._inventory;
    if (equipped) {
        for (var a = 0; a < this.members().length; ++a) {
            var actor = this.members()[a];
            if (!actor) continue;
