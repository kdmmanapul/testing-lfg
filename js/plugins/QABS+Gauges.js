...
(function() {
  ...
  Game_CharacterBase.prototype.showGauge = function() {
    ...
    Game_CharacterBase.prototype.killGauge = function () {
        return this.battler()._killHpBar;
    };
  //-----------------------------------------------------------------------------
  // Game_Event
  ...
  Sprite_Character.prototype.setBattler = function(battler) {
    ...
    //  }
    //this._gaugeSprite.setup(this._character, battler); // ¿œπ›∏˜ ∞‘¿Ã¡ˆ
      ...
      if (!this._bossGauge) {
        this._bossGauge = new Sprite_BossGauge();
          ...
          this._character._bossAdded = false;
          SceneManager._scene.addBossGauge(this._character);
      }
  };
  ...
  Sprite_Gauge.prototype.setupGauges = function() {
    ...
    Sprite_Gauge.prototype.setupGauges = function (character, battler) {
        ...
        this.addChild(this._top);
        //Frame
        this._hpFrame = new Sprite();
        this._hpFrame.x = -this._width / 2 - 13;
        this._hpFrame.y = -7;
        this.addChild(this._hpFrame)
        // Name
        this._name = new Sprite();
        var font = this.getFontSettings();
        this._name.bitmap = new Bitmap(this._width, font.size + 4);
  ...
  Sprite_Gauge.prototype.drawGauge = function() {
    ...
    Sprite_Gauge.prototype.drawFrame = function () {
        if (this._battler._bossHpBar == 'mid') var frameImage = ImageManager.loadSystem('boss_mid_Frame');
        else var frameImage = ImageManager.loadSystem('boss_frame');
        this._hpFrame.bitmap = frameImage;
    };

  Sprite_Gauge.prototype.drawName = function() {
    var name = this._battler.enemy().name;
      var h = this._name.bitmap.height;
      this._name.bitmap.drawText(name, (this._width - name.length * 12) / 2, 2, this._width, h);
  };
  ...
  Sprite_Gauge.prototype.showGauge = function() {
    ...
    Sprite_Gauge.prototype.killGauge = function () {
        return this._character.killGauge();
    };
  ...
  Sprite_Gauge.prototype.hideHud = function() {
    ...
    Sprite_Gauge.prototype.killHud = function () {
        if (this.visible) {
            if (this.opacity > 0) {
                this.opacity -= 10;
            } else {
                if (this._bossIndex != null) {
                    this.parent.killAndRefreshBossGauge(this);
                }
                this.clear();
                this.visible = false;
            }
    ...
    Sprite_Gauge.prototype.addBossGauge = function () {
    }

    Sprite_Gauge.prototype.refreshIndexOrder = function () {
        this.clear();
        if (!this._battler || !this.showGauge()) {
            return;
        }
  ...
  Sprite_BossGauge.prototype.updatePosition = function() {
      ...
      if (this.y != this._battler._bossHpBarOY + this._bossIndex * 24) {
          this.y += Math.round((this._battler._bossHpBarOY + this._bossIndex * 24 - this.y) / 10);
      }
  };

    Sprite_BossGauge.prototype.showGauge = function () {
      return !this._battler._hideHpBar;//this._character.inCombat();
  };

    Sprite_BossGauge.prototype.killGauge = function () {
        return this._battler._killHpBar;
    };
  ...
  Sprite_BossGauge.prototype.getFontSettings = function() {
    ...
    Sprite_BossGauge.prototype.setup = function (character, battler, index) {
        this._character = character;
        this._battler = battler;
        this.refresh();
    ...
    Scene_Map.prototype.createSpriteset = function () {
        ...
        $gameMap.events().forEach(function (event) {
            if (event._battler) {
                if (event.battler()._bossHpBar)
                    bossEvents.push(event);
            }
        })
        //var bossGauge = $gameMap.evnet(1)._bossGauge;// new Sprite_BossGauge();
        if (bossEvents.length > 0) {
            for (var i = 0; i < bossEvents.length; i++) {
                    bossEvents[i]._bossGauge = new Sprite_BossGauge();
                    bossEvents[i]._bossIndex = i;
                    bossEvents[i]._bossGauge.setup(bossEvents[i], bossEvents[i].battler(), i);
                    this.addChild(bossEvents[i]._bossGauge);
                    //this.parent.addChild(this._bossGauge);
            }
        }
    };

    Scene_Map.prototype.addBossGauge = function (character) {
        var i = 0;
        var characterId = character._eventId;
        this.children.forEach(function (sprite) {
            if (sprite._bossIndex != null) {
                i++;
                if (sprite._character._eventId == characterId) {
                    return;
                }
            }
        })
        character._bossGauge = new Sprite_BossGauge();
        character._bossIndex = i;
        character._bossGauge.setup(character, character.battler(), i);
        this.addChild(character._bossGauge);
        character._bossAdded = true;
    }

    Scene_Map.prototype.killAndRefreshBossGauge = function (child) {
