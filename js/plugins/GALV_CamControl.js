...
Galv.CC.camControl = function(args) {
	...
	switch (key) {
        ...
        case "boss":
            ...
            var target = $gameMap.event(eId);
            if (args[2]) speed = Galv.CC.getValue(args[2]);
            $gameMap._isTargetBoss = true;
            break;
        case "room":
            var px = Math.round($gameMap.width() / 2)//Galv.CC.getValue(args[1]);
            var py = Math.round($gameMap.height() / 2)//Galv.CC.getValue(args[2]);
            if (args[1]) speed = Galv.CC.getValue(args[1]);
            var target = {
                x: px,
                y: py,
    ...
    Galv.CC.startPlayerCam = function (speed) {
        var target = $gamePlayer;
        var spd = speed ? speed : 800;
        $gameMap.camTargetSet(target, spd);
    }

    Galv.CC.startEventCam = function (index, speed) {
        var target = $gameMap.event(index);
        var spd = speed ? speed : 800;
        $gameMap.camTargetSet(target, spd);
    }

    Galv.CC.startXYCam = function (x, y, speed) {
        var spd = speed? speed : 800;
        var target = {
            x: px,
            y: py,
    ...
    Galv.CC.startCamControl = function (target, speed, ) {
        var key = args[0].toLowerCase();
        var speed = 800;
        switch (key) {
            case "player":
...
Game_Map.prototype.updateScroll = function() {
    ...
    if (this._isTargetBoss) {
        var screenX = (this.camTarget.screenX() * 2 + $gamePlayer.screenX() * 3) / 5 * this.zoom.x;
        if (Math.abs(this.camTarget.screenX() - $gamePlayer.screenX()) > Graphics.width)
            screenX = $gamePlayer.screenX();
        var screenY = ((this.camTarget.screenY() * 2 + $gamePlayer.screenY() * 3) / 5 - 48) * this.zoom.y;
        if (Math.abs(this.camTarget.screenY() - $gamePlayer.screenY()) > Graphics.height)
            screenY = $gamePlayer.screenY();
    } else {
        var screenX = this._isTargetBoss ? (this.camTarget.screenX() * 2 + $gamePlayer.screenX() * 3) / 5 * this.zoom.x : this.camTarget.screenX() * this.zoom.x;
        var screenY = this._isTargetBoss ? (this.camTarget.screenY() * 2 + $gamePlayer.screenY()*3) / 5 * this.zoom.y : this.camTarget.screenY() * this.zoom.y;
    }
