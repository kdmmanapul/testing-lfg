...
Scene_Map.prototype.updatePictureEvents = function() {
    if (TouchInput.isTriggered() || Input.isTriggered('#esc') || Input.isTriggered('#e') || Input.isTriggered('$START') || Input.isTriggered('$A')) {
...
Scene_Map.prototype.getTriggeredPictureCommonEvent = function(check) {
    ...
    for (var i = 1; i < length; ++i) {
      ...
      lastpicture = picture;
        if (picture.isTriggered() || Input.isTriggered('#esc') || Input.isTriggered('#e') || Input.isTriggered('$START') || Input.isTriggered('$A')) return picture;
    }
    return null;
};
